...
A_StarPath::A_StarPath(const Field playGround) :
    m_PlayGround {playGround}, m_Start {0, 0}, m_End {0, 0} {
}
...
void A_StarPath::path()
{
    ...
    auto cmp = [&](const HistoricalPosition& p1, const HistoricalPosition& p2) -> bool {
        return p1.distance(m_End) + p1.step() > p2.distance(m_End) + p2.step();
    };
